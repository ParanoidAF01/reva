  ...
  Future<void> _fetchAndPrefill() async {
    ...
    try {
      ...
      if (response['success'] == true && response['data'] != null) {
        final spec = response['data']['specialization'] ?? {};
        if (spec['reraRegistered'] != null) {
          reraRegestration = spec['reraRegistered'] == true;
        }
        if ((spec['reraNumber'] ?? '').toString().isNotEmpty) {
          reraNUmber.text = spec['reraNumber'];
        }
        if ((spec['networkingMembers'] ?? '').toString().isNotEmpty) {
          networkingMember.text = spec['networkingMembers'];
        }
        if ((spec['realEstateWebsite'] ?? '').toString().isNotEmpty) {
          realEstateWebsite.text = spec['realEstateWebsite'];
        }
        if ((spec['associatedBuilders'] ?? '').toString().isNotEmpty) {
          associatedBuilders.text = spec['associatedBuilders'];
        }
      }
    } catch (_) {}
