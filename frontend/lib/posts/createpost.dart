...
class _SharePostScreenState extends State<SharePostScreen> {
  ...
  int usedPosts = 0;
  int maxPosts = 2;
  late Future myPostsFuture;

  @override
  void initState() {
    super.initState();
    myPostsFuture = ServiceManager.instance.posts.getMyPosts();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        final posts =
            (snapshot.data as Map<String, dynamic>)['data']?['posts'] ?? [];
        usedPosts = posts.length;

        return Scaffold(
          backgroundColor: const Color(0xFF22252A),
          // This is usually true by default but can be confirmed or set 
          // to allow resize on keyboard open to avoid overflow
          resizeToAvoidBottomInset: true,
          body: SafeArea(
            child: LayoutBuilder(
              builder: (context, constraints) {
                return SingleChildScrollView(
                  // Padding on the bottom for keyboard height
                              ...
                              children: [
                                Builder(
                                  builder: (context) {
                                    ...
                                    final profilePic = userProvider.userData?['profilePicture'] ??
                                    ...
                                    return CircleAvatar(
                                      ...
                                      backgroundImage:
                                          profilePic != null &&
                                                  ...
                                                  !profilePic.toString().contains('assets/')
                                              ? NetworkImage(profilePic)
                                              : const AssetImage('assets/dummyprofile.png')
                                                  as ImageProvider,
                                    );
                                  },
                                ),
                                          ...
                                          if (text.isNotEmpty || mediaUrl != null) {
                                            try {
                                              ...
                                              final postData = {
                                                ...
                                                'images': (pickedType == 'photo' &&
                                                        mediaUrl != null &&
                                                        mediaUrl.startsWith(
                                                            'https://res.cloudinary.com'))
                                                    ? [mediaUrl]
                                                    : [],
                                                'videos': (pickedType == 'video' &&
                                                        mediaUrl != null &&
                                                        mediaUrl.startsWith(
                                                            'https://res.cloudinary.com'))
                                                    ? [mediaUrl]
                              ...
                              children: [
                                ...
                                InkWell(
                                  onTap: () {
                                    setState(() {
                                      postController.text =
                                          "🎉 I'm celebrating a special occasion! Details: ";
                                    });
                                  },
                                  child: _buildOption(Iconsax.award, "Celebrate an occasion",
                                      highlight: false),
                                ),
                                InkWell(
                                  onTap: () {
                                    setState(() {
                                      postController.text =
                                          "📄 Sharing a document: [Document Name/Details]";
                                    });
                                  },
                                  child: _buildOption(Iconsax.document, "Add a document",
                                      highlight: false),
                                ),
                                InkWell(
                                  onTap: () {
                                    setState(() {
                                      postController.text =
                                          "💼 We're hiring! Position: [Role], Details: ";
                                    });
                                  },
                                  child: _buildOption(
                                      Iconsax.briefcase, "Share that you’re hiring",
                                      highlight: false),
                                ),
                                InkWell(
                                  onTap: () {
                                    setState(() {
                                      postController.text =
                                          "🔍 Looking for an expert in: [Field/Skill]";
                                    });
                                  },
                                  child: _buildOption(Iconsax.people, "Find an expert",
                                      highlight: false),
                                ),
                                InkWell(
                                  onTap: () {
                                  ...
                                  child: _buildOption(Iconsax.chart, "Create a poll",
                                      highlight: false),
                                ),
                              ],
                            ),
